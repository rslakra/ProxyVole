...
public class PacScriptMethods implements ScriptMethods {
	...
	public boolean isResolvable(String host) {
		try {
			...
			Logger.log(JavaxPacScriptParser.class, LogLevel.ERROR, "Hostname not resolveable! host:{0}.", host);
		}
		return false;
	}
	...
	public String dnsResolve(String host) {
		String dnsResolve = "";
		try {
			dnsResolve = InetAddress.getByName(host).getHostAddress();
		} catch(UnknownHostException e) {
			Logger.log(JavaxPacScriptParser.class, LogLevel.ERROR, "DNS name not resolvable! host:{0}.", host);
		}
		
		Logger.log(JavaxPacScriptParser.class, LogLevel.TRACE, "dnsResolve:{0}", dnsResolve);
		return dnsResolve;
	}
	...
	private String getLocalAddressOfType(Class<? extends InetAddress> cl) {
		try {
			...
			Logger.log(JavaxPacScriptParser.class, LogLevel.ERROR, "Local address not resolvable., error:{0}", e);
			return "";
		}
	}
	...
	public String dnsResolveEx(String host) {
		...
		Logger.log(JavaxPacScriptParser.class, LogLevel.TRACE, "result:{0}", result.toString());
	...
	public String sortIpAddressList(String ipAddressList) {
		...
		for(String ip : ipAddressToken) {
			try {
				...
				Logger.log(getClass(), LogLevel.ERROR, "error:{0}", e);
